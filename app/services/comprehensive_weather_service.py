...
class ComprehensiveWeatherService:
    ...
    async def get_comprehensive_weather_events(
        ...
        logger.info(f"Fetching comprehensive weather data for {start_date} to {end_date}")
        ...
        swdi_events = await self._fetch_swdi_data(
                ...
                for dataset_id in storm_datasets:
                    try:
                        ...
                        async with httpx.AsyncClient(timeout=self.timeout) as client:
                            ...
                            if results:
                                logger.info(f"Found {len(results)} storm events in {dataset_id}")
                        ...
                        if e.response.status_code == 400:
                            logger.debug(f"Dataset {dataset_id} not available or invalid parameters")
                        else:
                        ...
                        async with httpx.AsyncClient(timeout=self.timeout) as client:
                            ...
                            if data.get("result"):
                                for feature in data["result"]:
                                    event = self._convert_swdi_to_event(feature, endpoint)
                                    if event:
                                        events.append(event)
    ...
    def _convert_storm_event_to_event(self, record: Dict) -> Optional[SevereWeatherEvent]:
    ...
    def _convert_swdi_to_event(self, feature: Dict, endpoint: str) -> Optional[SevereWeatherEvent]:
    ...
    def _convert_enhanced_ghcnd_to_event(self, record: Dict) -> Optional[SevereWeatherEvent]:
    ...
    def _mark_insurance_relevance(self, events: List[SevereWeatherEvent]) -> List[SevereWeatherEvent]:
...
# Factory function
def get_comprehensive_weather_service(cdo_api_token: str) -> ComprehensiveWeatherService:
