...
class GeocodingService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "Google Maps API key not configured - geocoding will use fallback services"
            )
            self.api_key = None
    ...
    async def autocomplete_address(
        ...
        """
        try:
            params = {"input": input_text, "key": self.api_key, "types": types}
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.GOOGLE_MAPS_AUTOCOMPLETE_URL, params=params, timeout=10.0
                )
            ...
            for prediction in data.get("predictions", []):
                suggestion = AddressSuggestion(
                    ...
                    secondary_text=prediction["structured_formatting"].get(
                        "secondary_text"
                    ),
                )
            ...
            logger.info(
                f"Found {len(suggestions)} autocomplete suggestions for '{input_text}'"
            )

            return AutocompleteResponse(
                suggestions=suggestions, status=data.get("status", "OK")
            )
    ...
    async def geocode_address(
        self, address: Optional[str] = None, place_id: Optional[str] = None
    ) -> GeocodeResponse:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.GOOGLE_MAPS_GEOCODE_URL, params=params, timeout=10.0
                )
                response.raise_for_status()
                data = response.json()
            ...
            if status != "OK" or not data.get("results"):
                error_msg = data.get(
                    "error_message", f"Geocoding failed with status: {status}"
                )
                logger.warning(f"Geocoding failed: {error_msg}")
                return GeocodeResponse(
                    result=None, status=status, error_message=error_msg
                )
            ...
            logger.info(
                f"Geocoded address to {geocode_result.latitude}, {geocode_result.longitude}"
            )

            return GeocodeResponse(result=geocode_result, status=status)

        except httpx.HTTPError as e:
            logger.error(f"HTTP error during geocoding: {str(e)}")
            return GeocodeResponse(result=None, status="ERROR", error_message=str(e))
        except Exception as e:
            logger.error(f"Error during geocoding: {str(e)}")
            return GeocodeResponse(result=None, status="ERROR", error_message=str(e))

    async def reverse_geocode(
        self, latitude: float, longitude: float, result_type: Optional[str] = None
    ) -> ReverseGeocodeResponse:
        """
        ...
        try:
            ...
            if self.api_key:
                result = await self._reverse_geocode_with_google_maps(
                    latitude, longitude, result_type
                )
                if result.status == "OK" and result.results:
                    return result
                logger.warning(
                    f"Google Maps reverse geocoding failed for coordinates: {latitude}, {longitude}, trying fallback"
                )
    ...
    async def _reverse_geocode_with_google_maps(
        ...
        try:
            ...
            if status != "OK" or not data.get("results"):
                error_msg = data.get(
                    "error_message", f"Reverse geocoding failed with status: {status}"
                )
                logger.warning(f"Google Maps reverse geocoding failed: {error_msg}")
                return ReverseGeocodeResponse(
                    results=[], status=status, error_message=error_msg
                )
            ...
            logger.info(
                f"Google Maps reverse geocoded {latitude},{longitude} to {len(results)} results"
            )

            return ReverseGeocodeResponse(results=results, status=status)
    ...
    async def _reverse_geocode_with_nominatim(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    if data:
                        ...
                        geocode_result = self._parse_nominatim_result(
                            data, latitude, longitude
                        )

                        logger.info(
                            f"Nominatim reverse geocoded {latitude},{longitude} successfully"
                        )

                        return ReverseGeocodeResponse(
                            results=[geocode_result], status="OK"
                        )

                logger.warning(
                    f"Nominatim reverse geocoding failed for coordinates: {latitude}, {longitude}"
                )
                return ReverseGeocodeResponse(
                    results=[],
                    status="ZERO_RESULTS",
            ...
            logger.error(
                f"Error with Nominatim reverse geocoding for coordinates ({latitude}, {longitude}): {e}"
            )
            return ReverseGeocodeResponse(
                results=[], status="ERROR", error_message=str(e)
            )
    ...
    def _parse_nominatim_result(
        ...
        if city:
            address_components.append(
                AddressComponent(long_name=city, short_name=city, types=["locality"])
            )
        if state:
            address_components.append(
                AddressComponent(
    ...
    def _parse_geocode_result(self, result_data: Dict[str, Any]) -> GeocodeResult:
        ...
        geocode_result = GeocodeResult(
            ...
            county=component_map.get("administrative_area_level_2", {}).get(
                "long_name"
            ),
            state=component_map.get("administrative_area_level_1", {}).get("long_name"),
            state_code=component_map.get("administrative_area_level_1", {}).get(
                "short_name"
            ),
