...
class WeatherDataService:
    ...
    async def get_current_weather(
        ...
        """
        try:
            return await self.noaa_service.get_current_weather(
                latitude, longitude, location_name
            )

        except Exception as e:
            logger.error(
                f"Error fetching current weather for {latitude}, {longitude}: {e}"
            )
    ...
    async def get_weather_forecast(
        ...
        """
        try:
            return await self.noaa_service.get_weather_forecast(
    ...
    async def get_historical_weather(
        ...
        try:
            ...
            if not (
                180 <= period_days <= 185
                or 270 <= period_days <= 275
                or 720 <= period_days <= 735
            ):
                raise ValueError(
                    "Analysis period must be exactly 6 months, 9 months, or 24 months"
                )
            ...
            logger.error(
                f"Error fetching historical weather for {latitude}, {longitude}: {e}"
            )
            raise
    ...
    async def get_weather_events(
        ...
        try:
            ...
            logger.error(
                f"Error fetching weather events for {latitude}, {longitude}: {e}"
            )
            raise
    ...
    async def get_hail_probability(
        ...
        try:
            ...
            logger.error(
                f"Error fetching hail probability for {latitude}, {longitude}: {e}"
            )
            return {
                "hail_probability": [],
                "source": "Error",
                "note": "Hail probability unavailable",
            }
    ...
    async def get_fire_risk_assessment(
        ...
        try:
            ...
            logger.error(
                f"Error fetching fire risk assessment for {latitude}, {longitude}: {e}"
            )
            return {
                "fire_index": [],
                "source": "Error",
                "note": "Fire risk assessment unavailable",
            }
        ...
        try:
            ...
            if wind_speeds:
                trends["wind"] = {
                    ...
                    "max": max(wind_speeds),
                    "actionable_wind_days": len(
                        [w for w in wind_speeds if w >= WIND_MIN_ACTIONABLE]
                    ),  # ≥60 mph
                    "severe_wind_days": len(
                        [w for w in wind_speeds if w >= WIND_SEVERE]
                    ),  # ≥60 mph
    ...
    def _analyze_seasonal_patterns(
        self, observations: List[Dict[str, Any]], start_date: date, end_date: date
    ) -> Dict[str, Any]:
            ...
            for month, data in monthly_data.items():
                ...
                if data["precipitations"]:
                    patterns["precipitation"] = {
                        ...
                        "average": sum(data["precipitations"])
                        / len(data["precipitations"]),
    ...
    def _analyze_period_patterns(
        self, observations: List[Dict[str, Any]], period_days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            precipitation = [
                obs.get("precipitation_intensity", 0) for obs in observations
            ]
            wind_speeds = [
                obs.get("wind_speed") for obs in observations if obs.get("wind_speed")
            ]
            ...
            if temperatures:
                analysis["temperature"] = {
                    ...
                    "range": max(temperatures) - min(temperatures),
                    "extreme_days": len(
                        [t for t in temperatures if t < 32 or t > 90]
                    ),  # Freezing or hot days
                }
            ...
            if wind_speeds:
                analysis["wind"] = {
                    ...
                    "moderate_wind_days": len(
                        [w for w in wind_speeds if w >= WIND_MODERATE]
                    ),  # ≥40 mph
