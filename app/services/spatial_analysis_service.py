...
class SpatialAnalysisService:
    ...
    async def analyze_spatial_area(
        ...
        try:
            ...
            boundary_coords = await self._get_boundary_coordinates(boundary_type, boundary_data)
            
            # Generate analysis grid points within boundary
            grid_points = await self._generate_analysis_grid(boundary_coords, boundary_type)
            ...
            return {
                ...
                "route_optimization": await self._optimize_routes(grid_points, analysis_options),
    ...
    async def _get_boundary_coordinates(
        ...
        if boundary_type == "county":
            ...
            return await self._get_county_boundary(county_name, boundary_data.get("state_code"))
            ...
            return await self._get_city_boundary(city_name, boundary_data.get("state_code"))
        elif boundary_type == "neighborhood":
            neighborhood_name = boundary_data.get("neighborhood_name") or boundary_data.get("name")
            if not neighborhood_name:
                raise ValueError("Neighborhood name is required for neighborhood boundary type")
            return await self._get_neighborhood_boundary(neighborhood_name, boundary_data.get("city_name"))
    ...
    async def _get_county_boundary(self, county_name: str, state_code: Optional[str] = None) -> List[Tuple[float, float]]:
    ...
    async def _get_city_boundary(self, city_name: str, state_code: Optional[str] = None) -> List[Tuple[float, float]]:
    ...
    async def _get_neighborhood_boundary(self, neighborhood_name: str, city_name: Optional[str] = None) -> List[Tuple[float, float]]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                raise Exception(f"Could not find boundary for neighborhood: {neighborhood_name}")
    ...
    def _calculate_spatial_summary(self, grid_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "precipitation_stats": {
                ...
                "average": sum(precipitation) / len(precipitation) if precipitation else 0
            }
        }
    
    def _assess_spatial_risk(self, grid_data: List[Dict[str, Any]], analysis_options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "low_risk_areas": {
                "count": len(low_risk_areas),
                "areas": low_risk_areas
            }
        }
    
    def _analyze_spatial_events(self, grid_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _assess_business_impact(
        ...
        try:
            ...
            return {
                ...
                "recommendations": self._generate_business_recommendations(impact_score, impact_factors)
            }
            
        except Exception as e:
            logger.error(f"Error assessing business impact: {e}")
            return {"impact_score": 0, "impact_level": "low", "impact_factors": [], "recommendations": []}
    ...
    async def _identify_lead_opportunities(
        ...
        try:
            ...
            return {"lead_score": 0, "total_opportunities": 0, "opportunities": [], "lead_level": "low"}
    ...
    async def _optimize_routes(
        ...
        try:
            ...
            for i, cluster in enumerate(clusters):
                if len(cluster) > 1:
                    ...
                    optimized_routes.append({
                        ...
                        "estimated_distance_km": self._estimate_route_distance(cluster),
                        "efficiency_score": len(cluster) / max(1, self._estimate_route_distance(cluster))
                    })
            ...
            return {
                ...
                "optimization_score": sum(route["efficiency_score"] for route in optimized_routes) / max(1, len(optimized_routes))
            }
            
        except Exception as e:
            logger.error(f"Error optimizing routes: {e}")
            return {"total_routes": 0, "routes": [], "total_points": len(grid_points), "optimization_score": 0}
    
    def _cluster_points(self, points: List[Tuple[float, float]], max_distance_km: float) -> List[List[Tuple[float, float]]]:
