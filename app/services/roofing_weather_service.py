...
class RoofingWeatherService:
    ...
    async def get_roofing_weather_analysis(
        ...
        swdi_events = await self._fetch_swdi_radar_data(
        ...
        alert_events = await self._fetch_current_alerts(
            latitude, longitude
        )
    ...
    async def _fetch_swdi_radar_data(
        ...
        try:
            ...
            while current_start < end_date:
                chunk_end = min(
                    current_start + timedelta(days=31),  # SWDI 31-day limit
                    end_date
                )
                        ...
                        async with httpx.AsyncClient(timeout=self.timeout) as client:
                            ...
                            if data.get("result"):
                                for feature in data["result"]:
                                    event = self._convert_swdi_to_roofing_event(feature, event_type)
                                    if event:
                                        events.append(event)
    ...
    async def _simulate_spc_data(
        ...
        for event_data in simulated_events:
            event = SevereWeatherEvent(
                ...
                damage_potential="high" if event_data["severity"] == "severe" else "medium",
                insurance_relevant=True,
                roofing_damage_risk="high" if event_data["severity"] == "severe" else "medium"
            )
            events.append(event)
        
        return events
    
    def _convert_swdi_to_roofing_event(self, feature: Dict, event_type: str) -> Optional[SevereWeatherEvent]:
        ...
        try:
            ...
            return SevereWeatherEvent(
                ...
                damage_potential="high" if severity in ["severe", "extreme"] else "medium",
                insurance_relevant=True,
                roofing_damage_risk="high" if event_type in ["hail", "tornado"] else "medium"
            )
    ...
    def _convert_ghcnd_to_roofing_event(self, record: Dict) -> Optional[SevereWeatherEvent]:
    ...
    def _convert_alert_to_roofing_event(self, feature: Dict) -> Optional[SevereWeatherEvent]:
        ...
        try:
            ...
            return SevereWeatherEvent(
                ...
                damage_potential="high" if severity in ["severe", "extreme"] else "medium",
                insurance_relevant=True,
                roofing_damage_risk="high" if event_type in ["hail", "tornado", "wind"] else "medium"
            )
    ...
    def _deduplicate_and_sort_events(self, events: List[SevereWeatherEvent]) -> List[SevereWeatherEvent]:
    ...
    def _mark_roofing_relevance(self, events: List[SevereWeatherEvent]) -> List[SevereWeatherEvent]:
        ...
        for event in events:
            ...
            # Moderate events with certain types are relevant
                ...
                "hail", "tornado", "wind", "thunderstorm"
            ]:
                event.insurance_relevant = True
    ...
    def _generate_roofing_analysis(self, events: List[SevereWeatherEvent], latitude: float, longitude: float) -> Dict:
        ...
        risk_score = 0
        if total_events > 0:
            risk_score = (insurance_relevant_count * 2 + high_risk_count * 3) / total_events
        
        # Generate marketing recommendations
        recommendations = []
        if high_risk_count > 0:
            recommendations.append("High-risk property - prioritize for roofing marketing")
        if insurance_relevant_count >= 3:
            recommendations.append("Multiple insurance-relevant events - strong marketing potential")
        if "hail" in event_counts and event_counts["hail"] > 0:
            recommendations.append("Hail damage potential - focus on hail-resistant roofing")
        if "wind" in event_counts and event_counts["wind"] > 0:
            recommendations.append("Wind damage potential - emphasize wind-resistant materials")
        ...
        return {
            ...
            "roofing_marketing_score": min(10, max(1, int(risk_score * 2))),  # 1-10 scale
            "priority_level": "High" if risk_score > 2 else "Medium" if risk_score > 1 else "Low"
        }
